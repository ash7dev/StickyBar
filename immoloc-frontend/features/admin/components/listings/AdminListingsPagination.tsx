...
export function AdminListingsPagination({ page, totalPages, total, onPageChange }: AdminListingsPaginationProps) {
  ...
  return (
    <div className="flex items-center justify-between rounded-card border border-border bg-background-card px-4 py-3">
      <p className="text-xs text-foreground-muted">
        <span className="font-semibold text-foreground">{total}</span> annonce{total > 1 ? 's' : ''} au total — Page{' '}
