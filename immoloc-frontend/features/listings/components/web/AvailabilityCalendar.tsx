  ...
  return (
    <div className="flex items-center justify-between px-1 pb-3">
      <button
        ...
        className="w-8 h-8 rounded-full border-2 border-border flex items-center justify-center text-foreground-muted hover:border-success-500 hover:text-success-600 hover:bg-success-50 disabled:opacity-25 disabled:cursor-not-allowed transition-all"
...
export function AvailabilityCalendar({
      ...
      <div className="rdp-compact">
        ...
        <div className="flex items-center gap-2 mb-3">
          <div className={`flex-1 flex items-center gap-2.5 px-4 py-3 rounded-2xl border-2 transition-colors ${
            ...
            <CalendarDays className={`w-4 h-4 shrink-0 ${range?.from ? 'text-success-600' : 'text-foreground-muted'}`} />
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-foreground-muted leading-none mb-0.5">DÉPART</p>
              <p className={`text-sm font-bold leading-none ${range?.from ? 'text-success-700' : 'text-foreground-muted'}`}>
          ...
          <div className={`flex-1 flex items-center gap-2.5 px-4 py-3 rounded-2xl border-2 transition-colors ${
            ...
            <CalendarDays className={`w-4 h-4 shrink-0 ${range?.to ? 'text-success-600' : 'text-foreground-muted'}`} />
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-foreground-muted leading-none mb-0.5">RETOUR</p>
              <p className={`text-sm font-bold leading-none ${range?.to ? 'text-success-700' : 'text-foreground-muted'}`}>
        ...
        <div className="mt-3 pt-3 border-t border-border flex flex-wrap items-center gap-x-4 gap-y-2">
          {[
            ...
            { color: 'bg-success-500', label: 'Départ / Retour' },
            { color: 'bg-success-50', label: 'Plage réservée' },
            { color: 'bg-background-card border border-success-500', label: "Aujourd'hui" },
          ].map(({ color, label }) => (
        ...
        {range?.from && (
          <button
            ...
            className="mt-3 text-xs font-bold text-foreground-muted hover:text-success-600 transition-colors underline underline-offset-2"
          >
            Effacer les dates
          </button>
        )}
        ...
        <style>{`
          ...
          .rdp-compact .rdp-root {
            --rdp-accent-color: var(--success-500);
            --rdp-accent-background-color: var(--success-50);
          ...
          .rdp-compact .rdp-selected:not(.rdp-range_middle) .rdp-day_button {
            ...
            font-weight: 800;
            border-radius: 50%;
            box-shadow: 0 2px 8px rgba(46, 158, 82, 0.35);
          }
          /* Range middle */
  ...
  return (
    <div className="w-full max-w-full bg-background-card rounded-[1.5rem] border border-border overflow-hidden shadow-sm">
      {/* Header */}
      <div className="px-6 pt-6 pb-4 border-b border-border">
        <div className="flex items-center gap-2.5 mb-1">
          <CalendarDays className="w-4 h-4 text-success-600" />
      ...
      {range?.from && (
        <div className="px-6 pb-5">
          <button
            onClick={() => handleSelect(undefined)}
            className="text-xs font-bold text-foreground-muted hover:text-success-600 transition-colors underline underline-offset-2"
          >
            Effacer les dates
          </button>
        </div>
      ...
      <style>{`
        ...
        .rdp-klef .rdp-months::-webkit-scrollbar { display: none; }
        .rdp-klef .rdp-month { min-width: 100%; scroll-snap-align: start; }
        .rdp-klef .rdp-table { width: 100%; max-width: 100%; }
        .rdp-klef .rdp-day {
          border-radius: 0.5rem;
          font-size: 0.8125rem;
          font-weight: 500;
        ...
        .rdp-klef .rdp-day_button {
          width: 100%; height: 100%;
          border-radius: 0.5rem;
          display: flex; align-items: center; justify-content: center;
        }
        .rdp-klef .rdp-selected:not(.rdp-range_middle) .rdp-day_button,
        .rdp-klef .rdp-selected:not(.rdp-range_middle) {
          background: var(--success-600); color: var(--neutral-0); font-weight: 700; border-radius: 0.5rem;
        }
        .rdp-klef .rdp-range_middle { background: var(--success-50); color: var(--success-700); border-radius: 0; }
        .rdp-klef .rdp-range_start { border-radius: 0.5rem 0 0 0.5rem !important; }
        .rdp-klef .rdp-range_end   { border-radius: 0 0.5rem 0.5rem 0 !important; }
        .rdp-klef .rdp-range_start.rdp-range_end { border-radius: 0.5rem !important; }
        .rdp-klef .rdp-today .rdp-day_button { border: 1.5px solid var(--success-500); color: var(--success-600); font-weight: 700; }
        .rdp-klef .rdp-disabled .rdp-day_button { opacity: 0.3; cursor: not-allowed; }
        .rdp-klef .rdp-caption_label { font-size: 0.8125rem; font-weight: 800; color: var(--foreground); letter-spacing: -0.025em; }
        .rdp-klef .rdp-weekday { font-size: 0.625rem; font-weight: 700; color: var(--foreground-muted); text-transform: uppercase; letter-spacing: 0.05em; }
        .rdp-klef .rdp-nav_button {
          border: 1px solid var(--border); border-radius: 0.5rem;
          width: 1.75rem; height: 1.75rem;
          display: flex; align-items: center; justify-content: center;
          color: var(--foreground-muted); transition: border-color 150ms, color 150ms;
        }
        .rdp-klef .rdp-nav_button:hover { border-color: var(--success-500); color: var(--success-600); }
        @media (max-width: 640px) {
          .rdp-klef .rdp-months { flex-direction: column; gap: 2rem; }
          .rdp-klef .rdp-day { width: 2.5rem; height: 2.5rem; font-size: 0.875rem; }
        }
      `}</style>
    </div>
  );
