...
const GESTIONNAIRE_NAV_GROUPS: NavGroup[] = [
  {
    title: 'Vue d’ensemble',
    items: [
      { href: '/gestionnaire', label: 'Tableau de bord', icon: LayoutDashboard, exact: true },
    ],
  },
  {
    title: 'Propriétaires & Logements',
    items: [
  ...
  {
    title: 'Séjours & Opérations',
    items: [
      { href: '/gestionnaire/planning', label: 'Planning (Arrivées/Départs)', icon: CalendarDays },
      { href: '/gestionnaire/reservations', label: 'Réservations', icon: KeyRound },
      { href: '/gestionnaire/etats-des-lieux', label: 'États des lieux & Inspections', icon: ClipboardCheck },
    ],
  },
  {
    title: 'Finances & Configuration',
    items: [
      { href: '/gestionnaire/finances', label: 'Finances & Commissions', icon: Wallet },
      { href: '/gestionnaire/parametres', label: 'Paramètres Conciergerie', icon: Settings },
    ],
  },
];
...
function NavLink({ item, onClose }: { item: NavItem; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <item.icon
          className={cn(
            ...
            active ? 'text-forest-700' : 'text-foreground-muted group-hover:text-forest-700',
          )}
          aria-hidden="true"
        />
      ...
      {item.badge && (
        <span className="rounded-pill bg-forest-100 border border-forest-200 px-2 py-0.5 text-[0.625rem] font-semibold text-forest-800 tabular-nums">
          {item.badge}
        </span>
      )}
    </Link>
...
export function GestionnaireSidebar({ isOpen = false, onClose = () => {} }: GestionnaireSidebarProps) {
  ...
  const initials = ((prenom[0] ?? '') + (nom[0] ?? user?.email?.[0] ?? 'G')).toUpperCase() || 'GK';
  ...
  return (
    <>
      ...
      {/* Barre latérale portail gestionnaire */}
      <aside
        ref={panelRef}
        aria-label="Navigation du portail gestionnaire"
        className={cn(
      ...
      >
        ...
        <div className="shrink-0 space-y-4 border-b border-border p-5 pt-[calc(1.25rem+env(safe-area-inset-top,0px))]">
          <div className="flex items-center justify-between gap-2">
            <Link href="/gestionnaire" className="flex items-baseline gap-0.5">
              ...
              <span className="font-display text-2xl font-semibold text-forest-600" aria-hidden="true">.</span>
            </Link>

            <div className="flex items-center gap-2">
              <span className="rounded-pill bg-forest-900 border border-forest-700 px-2.5 py-0.5 text-[0.6875rem] font-semibold uppercase tracking-[0.1em] text-lime-300 flex items-center gap-1">
        ...
        {/* Liens de navigation par groupes */}
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          {GESTIONNAIRE_NAV_GROUPS.map((group) => (
        ...
        {/* Pied de la Sidebar — Carte utilisateur Gestionnaire & Déconnexion */}
        <div className="shrink-0 border-t border-border p-4 space-y-3 bg-background-alt/50">
          <div className="flex items-center gap-3 rounded-inner border border-border bg-background-card p-3 shadow-xs">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-[0.6875rem] text-foreground-muted">{user?.email ?? 'gestionnaire@klef.sn'}</p>
            </div>
          </div>
